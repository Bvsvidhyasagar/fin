...
const Topbar = ({ role, setRole, onMenuClick }) => {
  ...
  const pageTitle = routeLabels[pathname] ?? 'Fintrak'
      ...
      <div className="flex items-center gap-2 md:gap-3 flex-shrink-0">
        <div className="relative">
          <select
            ...
            className="appearance-none pl-3 pr-7 py-1.5 text-sm font-medium bg-slate-100 border border-slate-200 rounded-lg text-slate-700 cursor-pointer hover:bg-slate-200 transition-colors focus:outline-none focus:ring-2 focus:ring-sky-500/40"
          >
