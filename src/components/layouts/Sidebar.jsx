...
import { LayoutDashboard, ArrowLeftRight, BarChart3, Wallet, X } from 'lucide-react'
  ...
  return (
    <>
      ...
      >
        ...
        <nav className="flex-1 px-3 flex flex-col gap-1">
          {navItems.map(({ to, label, icon: Icon }, i) => (
            ...
            >
              {({ isActive }) => (
                <>
                  <span className={`transition-colors ${isActive ? 'text-sky-400' : 'text-slate-500'}`}>
                    <Icon size={18} />
                  </span>
                  <span>{label}</span>
                  {isActive && (
        ...
        <div className="p-4 m-3 mb-4 rounded-xl bg-white/5 border border-white/[0.08] flex items-center gap-3">
