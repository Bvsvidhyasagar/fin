    ...
    <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm animate-fade-up delay-300">
      ...
      <div className="flex flex-col sm:flex-row items-center gap-4">
        <ResponsiveContainer width={180} height={180}>
          <PieChart>
            <Pie data={data} dataKey="value" innerRadius={52} outerRadius={80} paddingAngle={3} strokeWidth={0}>
              {data.map((_, i) => (
          ...
          {data.map((item, i) => {
            ...
            return (
              <div key={item.name} className="flex items-center gap-2.5">
                ...
                <span className="text-xs font-semibold text-slate-800 flex-shrink-0">{pct}%</span>
              </div>
            )
          })}
        </div>
