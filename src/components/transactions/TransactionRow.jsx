...
const TransactionRow = ({ txn }) => {
  ...
  return (
    <tr className="border-b border-slate-50 hover:bg-slate-50/60 transition-colors">
      ...
      <td className="py-3.5 px-4">
        <span className={`inline-flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-full ${txn.type === 'income' ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-600'}`}>
          {txn.type === 'income' ? <TrendingUp size={11} /> : <TrendingDown size={11} />}
          {txn.type.charAt(0).toUpperCase() + txn.type.slice(1)}
        </span>
      </td>
      <td className={`py-3.5 px-4 text-sm font-semibold whitespace-nowrap ${txn.type === 'income' ? 'text-emerald-600' : 'text-red-500'}`}>
